...
function LayerItem({ capa }: { capa: typeof CAPAS[number] }) {
  ...
  return (
    <div className="flex items-center gap-2 w-full py-1">
      ...
      <span className="text-[11px] font-medium text-zinc-400 flex-1">{capa.label}</span>
      {isLoading ? (
        <div
        ...
        <span className="text-[10px] text-zinc-500 font-medium w-6 text-right">{capa.count}</span>
      )}
      <button
        type="button"
        role="switch"
...
export default function LayerPanel() {
  ...
  return (
    <div className="bg-zinc-900/90 backdrop-blur-xl border border-zinc-700/40 rounded-xl shadow-2xl w-60 p-3">
      {!$panelCollapsed ? (
        <div className="flex flex-col gap-2 w-full">
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center gap-1">
              <SvgIcon name="layers" size={14} color="#3b82f6" />
              <span className="text-[11px] font-bold text-zinc-400">Capas</span>
            </div>
            <button
